...
void rx_frame(int16_t in[], int bits[]) {
	...
	int extended = (FRAME_SIZE / CYCLES) + i;  // compute once
        ...
        decimated_frame[extended] = input_frame[(i * CYCLES)];
    ...
    /*
     * Algorithm to find the frequency error
     */
    complex float decframe[128] = {0.0f};

    float max = 0.0f;
    int index = 0;

    /*
     * Grab the first row of pilot symbols
     * Ignore the 8 rows of data symbols
     * 
     * decframe will be zero-filled at the end
     */
    for (int i = 0; i < PILOT_SYMBOLS; i++) {
        decframe[i] = decimated_frame[max_index];
    }

    /*
     * 1600 / 128 = 12.5 Hz per filter
     */
    ...
    for (int i = 0; i < 64; i++) {
        ...
        decframe[i] = decframe[i + 64];
        decframe[i + 64] = temp;
    }
    
#ifdef DEBUG3
    /*
     * So far this shows a typical spectrum
     * but it never shows a frequency error??
     */
    for (int i = 0; i < 128; i++) {
        fprintf(stderr, "%d, %f\n", i, cabsf(decframe[i]));
    }
...
int main(int argc, char** argv) {
    ...
    fft_forward = fft_alloc(128, 0, NULL, NULL);
    ...
    fbb_rx_rect = cmplxconj(TAU * (CENTER + 20.0f) / FS);   // 20 Hz RX freq error
